...
## 0C. P0-001 through P0-006 cumulative integration amendment

The active P0 delivery is integrated in two dependency-safe stages. Stage A combines P0-003's cross-platform CI repair with P0-005's policy truth while preserving one non-mutating verification ladder. The exact Stage A commit must then pass `validate-ubuntu`, `validate-windows`, and `validate-macos` through native build. Stage B may apply P0-004 and P0-006 only after that evidence exists. P0-006 remains REVIEW until GitHub enforcement and merge-behavior demonstrations are verified.

The standalone P0-005 verification script is superseded by the cumulative ladder because it would otherwise remove P0-003 gates and restore a mutating format command.

## 0D. P0-007 assurance-classification amendment

P0-007 establishes a machine-enforced assurance taxonomy before later test-hierarchy expansion. `system_test.py` is source-contract inspection only. Validator checks are categorized as `architecture_lint`, `source_contract`, `mixed`, `behavioral`, `sdk_toolchain`, `platform`, `release`, or `unclassified`. Source and mixed checks never count as behavioral proof; unknown classifications fail strict reporting.

P0-007 depends only on P0-001, so it may be integrated while P0-003 through P0-006 remain in review. It must preserve P0-002 trust retirement, P0-003's non-mutating verification ladder when present, P0-005 policy truth, P0-004 toolchain locks, and P0-006 governance. P8-001 remains responsible for the complete architecture/unit/component/integration/platform/adversarial/benchmark/release hierarchy.

## 0E. P0-008 bootstrap roadmap-control amendment

P0-008 establishes the first machine-enforced execution ledger for the P0/P1 bootstrap scope. `docs/roadmap/MASTER.md` remains the human engineering constitution, while `docs/roadmap/roadmap.yaml` is the dependency, status, packet, and evidence authority for P0 and P1. The manifest uses the dependency-free JSON subset of YAML 1.2 until P24 approves a richer all-task representation.

`STATUS.md` and `HANDOFF.md` are generated views and may not conflict with the manifest. CI must reject duplicate task IDs, missing dependencies, dependency cycles, invalid statuses, missing task packets or required evidence, a READY task whose dependencies are incomplete, a completed dependency set left as NOT_STARTED, or any second file claiming human roadmap authority. P0-008 creates proposed ADR stubs but does not approve the runtime, Owner Mode, signing, automation-host, browser-storage, or updater decisions assigned to later tasks.

P24 remains responsible for expanding this bootstrap into the complete all-task manifest, document split, acceptance-criterion lint, release-claim traceability, supersession graph, and bounded AI context-pack system.

## 0F. P0-009 initial benchmark amendment

P0-009 establishes the first versioned benchmark corpus and portable baseline for coding, analysis, path safety, crash recovery, browser-capability honesty, and research/citation tasks. The portable baseline is network-free, credential-free, model-free, and SDK-independent so a clean checkout can reproduce it byte-for-byte. Machine-specific and model-candidate runs are supplementary evidence and never replace the committed portable baseline.

A benchmark case must declare its assurance level and proof kind. Source inspection remains source-only. Unsupported, unavailable, not-run, failed, and infrastructure-error states remain visible and never count as passes. A P0-009 milestone pass means that the corpus, fixtures, evaluator, result semantics, and baseline are reproducible; it does not claim that Kristin passes every case or is production ready.

Browser tasks must remain `unsupported` until a real browser capability is implemented. Model-dependent coding, analysis, and research fixtures remain `not_run` until an exact candidate/provider run is supplied. Current failures, including generated-state policy gaps intended for P0-010, must not be deleted merely to improve the score. P8-001 later expands this bootstrap into the full benchmark and assurance hierarchy.
## 0G. P0-010 generated-state hygiene amendment

P0-010 establishes one generated-state policy for source validation, secret scanning, packaging, benchmark inputs, Git hygiene, and future automation-host work.

Normative rules:

1. `tool/source_tree_policy.py` is the generated-state classification authority.
2. Reviewed generated contracts under `lib/product/generated/` remain source inputs.
3. Durable milestone evidence under `release/evidence/<task-id>/` remains source input.
4. Python bytecode, Flutter/Dart package state, native ephemeral output, browser profiles/traces/downloads, Node caches, dataset intermediates, timestamped test reports, and root verification reports are disposable.
5. A managed `.gitignore` block is generated from the same policy and verified behaviorally with Git.
6. A Git checkout audits the index; a source archive audits `SOURCE_MANIFEST.sha256`.
7. A generated path that is still indexed but deleted from the worktree is a pending review deletion, never proof that the commit is clean.
8. Every automatic deletion receives an external backup and a content-hash receipt.
9. Standard verification must not rewrite source. Legacy mutating Dart formatting is converted to a check-only invocation when P0-003 has not already done so.
10. When P0-009 exists, P0-010 regenerates its portable baseline, records the before/after delta, requires `path_safety.generated_state_policy` to pass, and rejects regressions in previously passing cases.
11. CI must fail when a generated path is tracked, appears in the source manifest, or lacks canonical ignore coverage.
12. P9-002 may extend this policy for implemented browser/data subsystems but must not create a competing authority.

P0-010 does not rewrite Git history and does not remove reviewed evidence or generated source contracts.
...
- **3.1.6-p0-010-generated-state-hygiene** is this roadmap document's schema/content version.
...
```json
{
  "schemaVersion": "2.0.0",
  "taskId": "P11-003",
  "roadmapVersion": "3.1.4-p0-008-roadmap-control-plane",
  "commit": "git-sha",
...
Roadmap version: 3.1.4-p0-008-roadmap-control-plane
Current phase: P0
...
**Roadmap integration version:** `3.1.4-p0-008-roadmap-control-plane`
