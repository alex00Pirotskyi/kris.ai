...
## P0-003 integration correction — V3.1.1

The failed first integration attempt established additional executable rules:

1. POSIX-only imports must be guarded at module load and remain a required part of Linux sandbox readiness.
2. JSON/JSON-Schema-to-Dart generators share one reviewed string-literal encoder; generated files are never patched without their generator.
3. JSONPath and other dollar-bearing literals must be tested for Dart interpolation safety.
4. Dependency resolution precedes non-mutating format, analysis, and test gates.
5. CI patching must be structurally validated so generated steps cannot escape `jobs.<job>.steps` through indentation errors.
6. P0-004 remains blocked until the exact P0-003 commit passes Ubuntu, Windows, and macOS through native release build.
