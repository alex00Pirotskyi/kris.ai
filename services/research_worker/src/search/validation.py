...
def validate_freshness(value: Mapping[str, Any] | None) -> None:
    ...
    if mode == "between":
        ...
        if require_utc(after, "freshness.after") >= require_utc(
            before, "freshness.before"
        ):
...
def normalized_domains(
    ...
    return {
        "include": sorted(item.rstrip(".").lower() for item in value["include"]),
        "exclude": sorted(item.rstrip(".").lower() for item in value["exclude"]),
    }
