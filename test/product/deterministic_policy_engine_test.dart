...
void main() {
  Map<String, dynamic> readJson(String relative) {
    final path = <String>[Directory.current.path, ...relative.split('/')]
  ...
  setUpAll(() {
    ...
    fixture =
        readJson('evals/fixtures/p1_004_policy_engine/property_cases.json');
  });
  ...
  test('shared deterministic policy cases match in Dart', () {
    ...
    for (final raw in fixture['cases'] as List) {
      ...
      final result =
          engine.evaluate(Map<String, dynamic>.from(item['request'] as Map));
      expect(result['status'], item['expectedStatus'],
          reason: item['name'].toString());
      final reasons = Set<String>.from(result['reasonCodes'] as List);
      expect(reasons, containsAll(item['expectedReasons'] as List),
          reason: item['name'].toString());
    }
  });

  test('overlay ordering produces byte-equivalent decisions', () {
    final raw = (fixture['cases'] as List)
        .cast<Map>()
        .firstWhere((item) => item['name'] == 'overlay_order_reference');
    final request = Map<String, dynamic>.from(
        jsonDecode(jsonEncode(raw['request'])) as Map);
    final forward = engine.evaluate(request);
    request['overlays'] =
        (request['overlays'] as List).reversed.toList(growable: false);
  ...
  test('model text cannot approve or widen authority', () {
    final raw = (fixture['cases'] as List)
        .cast<Map>()
        .firstWhere((item) => item['name'] == 'model_cannot_approve');
    final result =
        engine.evaluate(Map<String, dynamic>.from(raw['request'] as Map));
  ...
  test('effective budgets are monotonic narrowing', () {
    final raw = (fixture['cases'] as List)
        .cast<Map>()
        .firstWhere((item) => item['name'] == 'project_write_inside_root');
