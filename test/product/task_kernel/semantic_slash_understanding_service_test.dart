...
void main() {
  ...
  test(
    '/fix free text is semantic while /run target-only stays deterministic',
    () {
  ...
  test(
    'slash capability and deterministic target survive model interpretation',
    () async {
      ...
      expect(
        outcome.specification.originalRequest,
        decision.parsed.originalText,
      );
      expect(outcome.specification.capabilityHints, <String>[
        'agent.fix_project',
      ]);
      expect(
  ...
  test(
    '/create and /search with semantic payloads warrant model understanding',
    () {
      final service = fixture(<String, dynamic>{});
      expect(
        service.warrantsModelUnderstanding(
          compiler.compile(
            '/create a small Flutter habit tracker with offline sync',
          ),
        ),
        isTrue,
      );
