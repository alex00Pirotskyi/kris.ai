...
void main() {
  test('product host adapter routes package effect through exact envelope',
      () async {
  ...
  test('screen bytes flow through product adapter without entering receipt',
      () async {
    final authority = TestEnvelopeAuthority();
    final journal = TestJournal();
    final bytes = utf8.encode('png-fixture');
    final client = TestAutomationHostClient((envelope) => <String, Object?>{
          'status': 'ok',
          'support': <String, Object?>{
            'status': 'supported',
    ...
    expect(jsonEncode(journal.receipts.single.toJson()),
