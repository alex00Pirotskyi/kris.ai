...
void main() {
  ...
  setUp(() async {
    ...
    ).writeAsString('<main>hello studio</main>\n', flush: true);
    await Directory('${root.path}/src').create();
    await File(
      '${root.path}/src/app.js',
    ).writeAsString('const message = "hello studio";\n', flush: true);
    await File(
      '${root.path}/src/styles.css',
    ).writeAsString('main { display: grid; }\n', flush: true);
    await Directory('${root.path}/node_modules/pkg').create(recursive: true);
    await File(
      '${root.path}/node_modules/pkg/ignored.js',
    ).writeAsString('const secretNoise = "hello studio";\n', flush: true);
  ...
  test(
    'file tree is bounded to project and skips generated dependency trees',
    () async {
  ...
  test(
    'new HTML CSS and JavaScript files save through mutation boundary',
    () async {
      ...
      expect(documents.map((item) => item.language), <P3WebStudioLanguage>[
  ...
  test(
    'search is bounded, line-aware and excludes generated directories',
    () async {
  ...
  test(
    'formatter diagnostics and source-control hooks are editor-facing',
    () async {
      ...
      expect(
        ...
        P3WebStudioDiagnosticSeverity.warning,
      );
