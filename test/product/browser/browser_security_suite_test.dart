...
void main() {
  test(
    'deterministic fixture is local-only and covers browser task surfaces',
    () {
      final html = File(
  ...
  test(
    'profile identifiers cannot escape the application-owned profile root',
    () async {
      ...
      final store = P3BrowserProfileStore(
        root: root,
        cipher: _RejectingCipher(),
      );
      await expectLater(
        store.put('../outside', const <String, Object?>{
          'cookies': <Object?>[],
        }),
