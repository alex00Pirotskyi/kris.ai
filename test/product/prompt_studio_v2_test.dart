...
void main() {
  ...
  final specification = ProductSpecificationV2.fromJson(
    fixture('specification.json'),
  );
  final policy = PlanCompilerPolicyV2.fromJson(
    fixture('policy.local_only.json'),
  );
  ...
  group('deterministic plan compiler', () {
    ...
    test(
      'rejects duplicate task IDs and dangling specification references',
      () {
        ...
        final invalidSpecification = deepCopy(fixture('specification.json'));
        final functional =
            invalidSpecification['functionalRequirements'] as List;
        final nonFunctional =
            invalidSpecification['nonFunctionalRequirements'] as List;
        nonFunctional.add(
          deepCopy(Map<String, dynamic>.from(functional.first as Map)),
        );
        ...
        (firstCriterion['evidenceValidatorIds'] as List).add(
          'validator_missing',
        );
    ...
    test(
      'blocks process execution while the v1.4 sandbox boundary is absent',
      () {
  ...
  group('prompt evaluation and revision impact', () {
    test('measures the fixture improvement without model execution', () {
      ...
      final impact = Map<String, dynamic>.from(
        comparison['measuredImpact'] as Map,
      );
