...
void main() {
  ...
  test('stale identity is replaced only after staged validation', () async {
    ...
    expect(leftovers.where((item) => item.path.contains('previous-')), isEmpty);
  });

  test(
    'invalid staged runtime never replaces a valid current runtime',
    () async {
  ...
  test(
    'interrupted previous runtime is restored before a new install',
    () async {
      final previous = File(
        '${root.path}${Platform.pathSeparator}runtime'
  ...
  test(
    'failed materialization is retryable and cleans in-flight state',
    () async {
  ...
  test(
    'same target identity shares one in-flight provisioning operation',
    () async {
      final runtimeSlot = slot();
