...
void main() {
  test('QA preview bridge is explicit and formally ineligible', () {
    ...
    final adapter =
        File('lib/product/p2_p1_authority_adapter.dart').readAsStringSync();
    final p1a = File('lib/product/p1_authority_service_contract_v1.dart')
        .readAsStringSync();
    final shell = File('lib/product/p2_app_shell.dart').readAsStringSync();
    final runtime = File('lib/product/p2_product_runtime_integration.dart')
        .readAsStringSync();
    final bootstrap = File('lib/product/p2_product_runtime_bootstrap.dart')
        .readAsStringSync();
    ...
    expect(runtime,
        contains('authority.completionEligible || authority.qaPreview'));
    expect(bootstrap, contains('allowQaPreview: qaPreview'));
    expect(shell, contains('QA PREVIEW — NOT RELEASE COMPLETE'));
  });
}
