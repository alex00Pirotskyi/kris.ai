...
void main() {
  ...
  test(
    'resolves model-authored capabilities before the family executor',
    () async {
      final plan = planWith(const <UniversalTask>[
        UniversalTask(
          id: 'retrieve',
  ...
  test(
    'coordinator capability never reaches a graph family executor',
    () async {
      final plan = planWith(const <UniversalTask>[
        UniversalTask(
          id: 'bad',
  ...
  test('reports queued running and terminal node states', () async {
    ...
    expect(states, <KernelTaskNodeState>[
