...
void main() {
  test(
    'live semantic patch replan boundary is explicit and authority neutral',
    () {
  ...
  test(
    'Chat follows continuation planning context without stale plan fallback',
    () {
      final source = File(
  ...
  test('null currentRun does not erase planning state before a run exists', () {
    ...
    expect(
      setter.contains('if (existing != null) {'),
      isTrue,
      reason:
  ...
  test(
    'Chat approval follows authoritative run state instead of a fixed delay',
    () {
      final studio = File(
        'lib/product/chat_control_plane_studio.dart',
      ).readAsStringSync();
