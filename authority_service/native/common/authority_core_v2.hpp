...
inline json::value evaluate_policy(const authority_config& cfg, const json::value& policy_request,
  ...
  const auto expected_approval_mac = crypto.hmac_sha256_hex("kristin.owner-approval.v2\0", key_id, json::canonical(approval_unsigned));
 ...
 private:
  ...
  json::value record_owner_approval(const json::value& request_value, const peer_identity& peer) {
    ...
    json::value::object approval{{"schemaVersion",json::value("2.0.0")},{"approvalId",json::value(approval_id)},
      {"approved",json::value(true)},{"source",json::value("owner")},{"binding",json::value(binding)},
    ...
    approval.at("auth").as_object().at("mac") = json::value(
      crypto_.hmac_sha256_hex("kristin.owner-approval.v2\0", config_.owner_approval_hmac_key_id,
  ...
  json::value authorize(const json::value& request_value, const peer_identity& peer) {
    ...
    const auto grant_id = "grant-" + sha256_hex(config_.service_instance_id + "|" + request_id + "|" + request_nonce).substr(0, 32);
    const auto grant_nonce = "nonce-" + sha256_hex(grant_id + "|" + request_nonce).substr(0, 32);
    ...
    const auto grant_mac = crypto_.hmac_sha256_hex("kristin.capability-grant.v2\0", config_.grant_hmac_key_id, json::canonical(unsigned_grant));
    ...
    if (!constant_time_equal(observed_mac, crypto_.hmac_sha256_hex("kristin.capability-grant.v2\0", config_.grant_hmac_key_id, json::canonical(verify_copy))))
    ...
    consumption.at("auth").as_object().at("mac")=json::value(crypto_.hmac_sha256_hex("kristin.grant-consumption.v1\0",config_.grant_hmac_key_id,json::canonical(unsigned_consumption)));
