        ...
        _transaction<void>(() {
          ...
          _database.execute(
            '''INSERT INTO agent_action_attempts(
               ...
               ON CONFLICT(
                 run_id, work_item_id, work_item_attempt, turn, request_number
               ) DO UPDATE SET
    ...
    generation = r'''      final deterministicCandidate =
      ...
      if (deterministicSelected) {
        ...
        final deterministicJson =
            attemptLedgerPolicy.actionJson(deterministicCandidate);
        ...
        await _bestEffortEvent(
          ...
          <String, dynamic>{
            ...
            'turn': turn + 1,
            'tool': deterministicCandidate.tool,
...
'''
    ...
    proposal = r'''      final actionJsonForLedger = attemptLedgerPolicy.actionJson(action);
      ...
      await repositories.workflow.recordAgentActionAttempt(
        ...
        beforeSha256: stateSha256,
        details: <String, dynamic>{
          'source': deterministicSelected ? 'deterministic' : 'model_or_recovery',
        },
      );
'''
...
def patch_tests_and_policy() -> None:
    helper = ROOT / "lib" / "product" / "runner_attempt_ledger.dart"
    text = helper.read_text(encoding="utf-8")
    text = replace_once(
        text,
        "if (character == r'\\\\') {",
        "if (character == '\\\\') {",
        "command tokenizer backslash",
    )
