...
def main():
 ...
 with tempfile.TemporaryDirectory(prefix='p2-runtime-contract-v63-') as td:
  ...
  # Compare canonical paths: hosted macOS/Windows may spell the same path differently.
  result=json.loads(r.stdout);manifest=pathlib.Path(result['manifest']);value=json.loads(manifest.read_text());assert manifest.parent.resolve()==out.resolve()
