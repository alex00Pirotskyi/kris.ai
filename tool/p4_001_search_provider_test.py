...
FORBIDDEN_IMPORTS = {
    "playwright", "selenium", "requests", "httpx", "aiohttp", "subprocess",
    "sqlite3", "psutil",
}
...
def main() -> int:
    ...
    assertions.extend((
        {"id": "P4-001.python.syntax", "passed": not syntax_errors,
         "detail": f"errors={syntax_errors}"},
        {"id": "P4-001.dependency.boundary", "passed": not forbidden,
    ...
    assertions.append({
        "id": "P4-001.fixture.contracts",
        "passed": result.wasSuccessful(),
        "detail": (
            f"testsRun={result.testsRun}, failures={len(result.failures)}, "
    ...
    report = {
        ...
        "sourceHashes": {
            path: sha256(project / path) for path in REQUIRED if (project / path).is_file()
        },
        "unittestOutput": output.getvalue(),
        "completionEligible": False,
        "completionReason": (
            "Worker C foundation only; independent review and repository CI are not bound."
        ),
    }
