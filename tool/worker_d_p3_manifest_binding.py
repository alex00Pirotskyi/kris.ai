...
def validate(root: Path) -> list[str]:
    ...
    for index, artifact in enumerate(artifacts):
        ...
        try:
            ...
            errors.append(f"manifest artifact missing or escapes repository {rel}")
            continue
        if target.is_symlink() or not target.is_file():
            errors.append(f"manifest artifact is not a regular source file {rel}")
            continue

        actual = hashlib.sha256(target.read_bytes()).hexdigest()
        if actual != expected:
            errors.append(
                f"manifest artifact digest mismatch {rel}: "
