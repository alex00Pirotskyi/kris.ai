...
def sem(kind: str, idx: int, **extra):
    item = {"kind": kind, "semaphoreId": f"SEM-{idx:04d}", "allowedPaths": [], "productPr": None, "authorityId": None, "resourceId": None}
    item.update(extra)
    return item


class SemaphoreScaleTests(unittest.TestCase):
    def test_100_non_overlapping_write_scopes_do_not_collide(self):
        items = [sem("WRITE", i, allowedPaths=[f"lib/product/shard_{i}/**"]) for i in range(100)]
        for i, left in enumerate(items):
            for right in items[i + 1:]:
                self.assertFalse(semaphore_collides(left, right))

    def test_same_write_scope_collides(self):
        self.assertTrue(semaphore_collides(sem("WRITE", 1, allowedPaths=["lib/product/model/**"]), sem("WRITE", 2, allowedPaths=["lib/product/model/a.dart"])))

    def test_integration_is_product_pr_scoped(self):
        self.assertTrue(semaphore_collides(sem("INTEGRATION", 1, productPr=76), sem("INTEGRATION", 2, productPr=76)))
        self.assertFalse(semaphore_collides(sem("INTEGRATION", 1, productPr=76), sem("INTEGRATION", 2, productPr=62)))

    def test_release_is_resource_scoped(self):
        self.assertTrue(semaphore_collides(sem("RELEASE", 1, resourceId="SOURCE_MANIFEST"), sem("RELEASE", 2, resourceId="SOURCE_MANIFEST")))
        self.assertFalse(semaphore_collides(sem("RELEASE", 1, resourceId="SOURCE_MANIFEST"), sem("RELEASE", 2, resourceId="INSTALLER_SIGNING")))
