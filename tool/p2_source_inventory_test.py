...
def main():
 ...
 production=data.get('productionDart');tests=data.get('testDart');support=data.get('supportDart');require(isinstance(production,list) and isinstance(tests,list) and isinstance(support,list) and support,'P2 inventories missing');require(len(production)==len(set(production)) and len(tests)==len(set(tests)) and len(support)==len(set(support)),'duplicates')
 ...
 for rel in production+tests+support:require((root/rel).is_file(),f'governed P2 source missing: {rel}')
