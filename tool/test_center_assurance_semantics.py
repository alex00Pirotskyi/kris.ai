...
def registry_ids(registry: dict[str, Any]) -> set[str]:
    cases = registry.get("testCases", [])
    profiles = registry.get("projectTestProfiles", [])
    ...
    invalid = sorted(
        ...
        if not isinstance(value, str) or not TEST_ID.fullmatch(value)
    )
    if invalid:
...
def validate_documents(
    ...
    if (
        schema.get("$id")
        != "https://local.kristin/schemas/test_center_assurance_hierarchy.v1.json"
        or schema.get("additionalProperties") is not False
    ):
        fail("hierarchy schema identity or closure drifted")
    if (
        report_schema.get("$id")
        != "https://local.kristin/schemas/test_center_assurance_execution_report.v1.json"
    ...
    if (
        ...
        .get("executionResult", {})
        .get("$ref")
        != "test_center.v1.json#/$defs/TestExecutionResult"
    ):
        fail("assurance execution report must bind canonical TestExecutionResult")
    schema_validate(hierarchy, schema)
    if (
        hierarchy.get("schemaVersion") != "1.0.0"
        or hierarchy.get("hierarchyId") != "test-center.assurance-hierarchy-v1"
        or hierarchy.get("roadmapTaskId") != "P8-001"
    ):
        fail("hierarchy identity is not canonical")
    levels = hierarchy["levels"]
    ids = [item["levelId"] for item in levels]
    ranks = [item["rank"] for item in levels]
    ...
    for item in levels:
        if len(item["requiredEvidence"]) < 2 or len(item["requiredEvidence"]) != len(
            set(item["requiredEvidence"])
        ):
            fail(f"required evidence is incomplete for {item['levelId']}")
        for predecessor in item["requiredPredecessorLevels"]:
            if (
                predecessor not in by_level
                or by_level[predecessor]["rank"] >= item["rank"]
            ):
                fail(f"invalid predecessor {predecessor!r} for {item['levelId']}")
    if (
        levels[0]["proofKind"] != "SOURCE_ONLY"
        or levels[0]["supportClaimCeiling"] != "NONE"
    ):
        fail("source-only architecture lint cannot promote support")
    if set(levels[-1]["requiredPredecessorLevels"]) != {
        "platform",
        "adversarial",
        "benchmark",
    }:
    ...
    if not REPORT_FIELDS.issubset(contract["requiredFields"]) or len(
        contract["requiredFields"]
    ) != len(set(contract["requiredFields"])):
        fail("assurance reports omit required fields or contain duplicates")
    if (
        contract["unknownLevelPolicy"] != "FAIL"
        or contract["crossLevelPromotionPolicy"] != "FORBIDDEN"
        or contract["sourceOnlySupportPromotion"] != "FORBIDDEN"
    ):
        fail("assurance report policies must fail closed")
    if (
        set(contract["unexecutedResultStates"]) != UNEXECUTED
        or contract["executionReportSchema"] != REPORT_SCHEMA.as_posix()
    ):
    ...
    duplicates = sorted(
        key for key, count in Counter(binding_ids).items() if count > 1
    )
    missing = sorted(case_ids - set(binding_ids))
    extra = sorted(set(binding_ids) - case_ids)
    ...
    for test_id in (
        ...
        if (
            not case
            or case.get("moduleId") != "tm.reliability-security-diagnostics"
            or "P8-001" not in case.get("roadmapTaskIds", [])
        ):
    ...
    return {
        ...
        "roadmapTaskId": "P8-001",
        "hierarchyId": hierarchy["hierarchyId"],
        "levelCount": len(levels),
        "bindingCount": len(bindings),
        "pendingMigrationBindingCount": len(pending),
        "levels": [
            {
                "assuranceLevel": item["levelId"],
                "rank": item["rank"],
                "proofKind": item["proofKind"],
                "supportClaimCeiling": item["supportClaimCeiling"],
            }
            for item in levels
        ],
        "unmappedTestIds": [],
        "unknownLevelPolicy": contract["unknownLevelPolicy"],
        "crossLevelPromotionPolicy": contract["crossLevelPromotionPolicy"],
        "sourceOnlySupportPromotion": contract["sourceOnlySupportPromotion"],
...
def validate_assurance_execution_report(
    ...
    *,
    ...
    try:
        ...
        raise HierarchyError(f"canonical TestExecutionResult invalid: {exc}") from exc

    canonical_evidence = result.get("evidenceReferences", [])
    canonical_evidence_ids = [item["evidenceId"] for item in canonical_evidence]
    wrapper_evidence_ids = report["evidenceReferences"]
    if len(canonical_evidence_ids) != len(set(canonical_evidence_ids)):
        fail("canonical execution result contains duplicate evidence IDs")
    if len(wrapper_evidence_ids) != len(set(wrapper_evidence_ids)):
        fail("assurance execution report contains duplicate evidence IDs")
    if set(wrapper_evidence_ids) != set(canonical_evidence_ids):
        missing_wrapper_ids = sorted(set(canonical_evidence_ids) - set(wrapper_evidence_ids))
        extra_wrapper_ids = sorted(set(wrapper_evidence_ids) - set(canonical_evidence_ids))
        fail(
            "assurance execution report evidence IDs do not exactly bind canonical "
            f"evidence objects; missing={missing_wrapper_ids}, extra={extra_wrapper_ids}"
        )

    active = {item["testId"]: item for item in hierarchy["testBindings"]}
    levels = {item["levelId"]: item for item in hierarchy["levels"]}
    test_id = result["testId"]
    level = report["assuranceLevel"]
    ...
    case = next(
        (item for item in registry["testCases"] if item["testId"] == test_id),
        None,
    )
    if case is None:
    ...
    if sorted(result["roadmapTaskIds"]) != sorted(case["roadmapTaskIds"]) or sorted(
        wrapper["roadmapTaskIds"]
    ) != sorted(case["roadmapTaskIds"]):
        fail("assurance execution report roadmap binding mismatch")
    if (
        report["candidateCommit"] != result["commit"]
        or report["candidateTree"] != result["tree"]
    ):
    ...
    if level not in CLASS_LEVELS.get(result["assuranceClass"], set()):
        fail(
            f"canonical assuranceClass {result['assuranceClass']!r} is incompatible "
            f"with {level!r}"
        )
    ...
    requested = report["requestedSupportImpact"]
    ceiling = levels[level]["supportClaimCeiling"]
    ...
    if requested != "NONE":
        if not canonical_evidence:
            fail("support-bearing PASS reports require candidate-bound immutable evidence")
        if result["cleanupState"] in {"DIRTY", "UNRESOLVED"}:
            fail("support-bearing PASS reports require resolved cleanup")
        if result["failureClassification"] != "NONE":
            fail("support-bearing PASS reports cannot carry a failure classification")
        if result["certificationImpact"] != "NONE":
            fail("support-bearing PASS reports cannot carry certification impact")
    return {
        "schemaVersion": "1.0.0",
        "status": "PASS",
        "checkMode": "NON_MUTATING",
        ...
        "requestedSupportImpact": requested,
