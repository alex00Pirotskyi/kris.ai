...
'''    const bool session_scope = approval_scope == "owner-session";
    ...
    if (!valid_identifier(request_id) || !valid_identifier(approval_id) || !valid_identifier(interaction_nonce) ||
        ...
        (!session_scope && !owner_session_id.empty()) || interaction_type != "native-owner-confirmation" || !user_present ||
        !valid_hex64(payload_sha) || !valid_hex64(ui_sha) || !valid_hex64(confirmation_sha) ||
        expires_at <= now || expires_at > now + max_approval_lifetime)
      throw std::runtime_error("owner_approval_request_invalid");''')
rep(p,
...
'''    if (json::required_string(approval_object, "schemaVersion") != "2.0.0" ||
    ...
    if (approval_scope == "effect") {
      ...
      const auto owner_session_id = request.contains("ownerSessionId")
          ? json::required_string(request, "ownerSessionId") : std::string();
      const auto approved_session_id = approval_object.contains("ownerSessionId") && !approval_object.at("ownerSessionId").is_null()
      ...
      if (json::required_string(approval_binding, "accessProfileId") != json::required_string(request_binding, "accessProfileId"))
        throw std::runtime_error("authority_owner_approval_binding_mismatch");
    } else {
...
'''    const auto grant_id = "grant-" + sha256_hex(config_.service_instance_id + "|" + owner_approval_id + "|" + approval_digest + "|" + request_id).substr(0, 32);''')
...
session='''  mock_state session_state; kp::authority_core session_core(config(),crypto,session_state);
  ...
  if (kp::json::required_string(session_authorized_1.as_object(),"status")!="authorized") throw std::runtime_error("session_authorization_first_failed");
  ...
  if (kp::json::required_string(session_authorized_2.as_object(),"status")!="authorized") throw std::runtime_error("session_authorization_second_failed");
...
# Runtime eligibility is not certification: keep completion false while exposing secure isolation readiness.
for p in ['authority_service/install/windows/Install-KristinP1Authority.ps1','authority_service/install/linux/install_linux.sh']:
 s=rd(p)
 if p.endswith('.ps1'):
  a="provenance=@{authorityType='p1-isolated-authority-service-v2';p1AmendmentMerged=$false;"
  b="provenance=@{authorityType='p1-isolated-authority-service-v2';runtimeEligible=$true;securityIsolationActive=$true;privateAuthorityMaterialPresent=$false;arbitraryMessageSigningApi=$false;p1AmendmentMerged=$false;"
 else:
  a='\\"provenance\\":{\\"authorityType\\":\\"p1-isolated-authority-service-v2\\",\\"p1AmendmentMerged\\":false'
  b='\\"provenance\\":{\\"authorityType\\":\\"p1-isolated-authority-service-v2\\",\\"runtimeEligible\\":true,\\"securityIsolationActive\\":true,\\"privateAuthorityMaterialPresent\\":false,\\"arbitraryMessageSigningApi\\":false,\\"p1AmendmentMerged\\":false'
 if a not in s: raise SystemExit(f'{p}: provenance anchor missing')
 wr(p,s.replace(a,b,1))
