...
class ManifestBindingTests(unittest.TestCase):
    ...
    def test_duplicate_binding_fails(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.write_manifest(root, manifest)
            errors = MODULE.validate(root)
            self.assertTrue(any("duplicate manifest artifact path" in error for error in errors))
    ...
    def test_unexpected_binding_fails(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            manifest["artifacts"].append(
                {"path": extra_rel, "sha256": sha256(extra.read_bytes()).hexdigest()}
            )
            self.write_manifest(root, manifest)
            errors = MODULE.validate(root)
            self.assertTrue(
