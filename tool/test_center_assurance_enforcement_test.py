...
class AssuranceEnforcementTest(unittest.TestCase):
    ...
    def validate_documents(self, **overrides) -> dict:
        return validate_documents(
            overrides.get("hierarchy_schema", self.hierarchy_schema),
            overrides.get("report_schema", self.report_schema),
            overrides.get("canonical", self.canonical),
            overrides.get("hierarchy", self.hierarchy),
            overrides.get("registry", self.registry),
            overrides.get("contract_schema", self.contract_schema),
    ...
    def validate_report(self, report: dict, *, hierarchy: dict | None = None) -> dict:
        return validate_assurance_execution_report(
            ...
            canonical_schema=self.canonical,
    ...
    def test_empty_toolchain_components_fail_min_properties(self) -> None:
        ...
        report["executionResult"]["toolchain"]["components"] = {}
        with self.assertRaisesRegex(HierarchyError, "too few properties"):
            self.validate_report(report)

    def test_naive_timestamp_rejected(self) -> None:
        report = copy.deepcopy(self.report)
        report["executionResult"]["startedAt"] = "2026-08-06T10:30:00"
        with self.assertRaisesRegex(HierarchyError, "RFC3339"):
            self.validate_report(report)

    def test_space_separated_timestamp_rejected(self) -> None:
        report = copy.deepcopy(self.report)
        report["executionResult"]["endedAt"] = "2026-08-06 10:30:01Z"
        with self.assertRaisesRegex(HierarchyError, "RFC3339"):
            self.validate_report(report)

    def test_generated_at_must_follow_execution(self) -> None:
        report = copy.deepcopy(self.report)
        report["generatedAt"] = "2026-08-06T10:29:59Z"
        with self.assertRaisesRegex(HierarchyError, "generatedAt precedes"):
            self.validate_report(report)

    def test_missing_evidence_target_rejected(self) -> None:
        report = copy.deepcopy(self.report)
        report["executionResult"]["evidenceReferences"][0]["uri"] = "release/evidence/TEST_CENTER/P8-001/fixtures/does-not-exist.json"
        with self.assertRaisesRegex(HierarchyError, "does not resolve"):
            self.validate_report(report)

    def test_evidence_digest_mismatch_rejected(self) -> None:
        report = copy.deepcopy(self.report)
        report["executionResult"]["evidenceReferences"][0]["sha256"] = "f" * 64
        with self.assertRaisesRegex(HierarchyError, "digest mismatch"):
            self.validate_report(report)

    def test_missing_required_evidence_category_rejected(self) -> None:
        report = copy.deepcopy(self.report)
        report["evidenceBindings"] = report["evidenceBindings"][:-1]
        with self.assertRaisesRegex(HierarchyError, "evidence categories"):
            self.validate_report(report)

    def test_duplicate_evidence_category_rejected(self) -> None:
        report = copy.deepcopy(self.report)
        report["evidenceBindings"][2]["category"] = report["evidenceBindings"][0]["category"]
        with self.assertRaisesRegex(HierarchyError, "duplicate evidence categories"):
            self.validate_report(report)
    ...
    def test_report_contract_schema_path_drift_rejected(self) -> None:
        ...
        contract["hierarchyRequiredFieldMappings"]["durationMillis"] = "executionResult.notAField"
        with self.assertRaisesRegex(HierarchyError, "absent from closed schema"):
            self.validate_documents(contract=contract)

    def test_report_schema_required_field_missing_from_contract_rejected(self) -> None:
        contract = copy.deepcopy(self.contract)
        contract["reportRequiredTopLevelFields"].remove("evidenceBindings")
        with self.assertRaisesRegex(HierarchyError, "required fields and mapping contract drifted"):
            self.validate_documents(contract=contract)
