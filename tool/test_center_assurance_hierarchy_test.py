...
def pending(test_id, **updates):
    def mutate(value):
        next(item for item in value["pendingMigrationBindings"] if item["testId"] == test_id).update(updates)
    return mutate
...
DOCUMENT_REGRESSIONS = [
    ...
    ("invalid predecessor", put(("levels", 2, "requiredPredecessorLevels"), ["release"])),
    ("missing binding", lambda value: value["testBindings"].pop()),
    ("duplicate binding", duplicate(("testBindings",))),
    ("architecture promotion", put(("levels", 0, "supportClaimCeiling"), "SOURCE_FOUNDATION")),
    ("release predecessors", put(("levels", 7, "requiredPredecessorLevels"), ["platform"])),
    ("missing report schema", pop(("reportContract", "executionReportSchema"))),
    ...
    ("p1a exit gate platform", pending("tc.p1a.exit-gate", levelId="platform")),
...
REPORT_REGRESSIONS = [
    ...
    ("above ceiling", put(("requestedSupportImpact",), "BEHAVIOR_SUPPORTED")),
    ...
    ("canonical extra field", put(("executionResult", "assuranceLevel"), "unit")),
    ...
    ("duration mismatch", put(("executionResult", "durationMillis"), 2000)),
    ("endedAt precedes startedAt", put(("executionResult", "endedAt"), "2026-08-06T10:29:59Z")),
    ("cross-candidate immutable evidence", put(("executionResult", "evidenceReferences", 0, "commit"), "3" * 40)),
    ("dirty cleanup promotion", put(("executionResult", "cleanupState"), "DIRTY")),
    ("unresolved cleanup promotion", put(("executionResult", "cleanupState"), "UNRESOLVED")),
    ("PASS failure contradiction", put(("executionResult", "failureClassification"), "ASSERTION")),
    ("PASS certification contradiction", put(("executionResult", "certificationImpact"), "BLOCKS_SCOPE")),
    ("wrapper evidence mismatch", put(("evidenceReferences", 0), "evidence.p8-001.mismatch")),
    ("wrapper evidence extra", append_wrapper_evidence),
    ("wrapper evidence missing", put(("evidenceReferences",), [])),
    ("canonical evidence missing wrapper binding", append_distinct_canonical_evidence),
    ("duplicate canonical evidence ID", duplicate_canonical_evidence_id),
    ("duplicate wrapper evidence ID", duplicate(("evidenceReferences",))),
    ("support-bearing report without canonical evidence", put(("executionResult", "evidenceReferences"), [])),
]
...
class AssuranceHierarchyTest(unittest.TestCase):
    ...
    def validate_documents(self, value):
        return H.validate_documents(self.hierarchy_schema, self.report_schema, value, self.registry)

    def validate_report(self, value, *, hierarchy=None, proof_contract=None):
        return H.validate_assurance_execution_report(value, report_schema=self.report_schema, canonical_schema=self.canonical_schema, hierarchy=hierarchy or self.hierarchy, registry=self.registry, project=PROJECT, proof_contract=proof_contract or self.proof_contract)
    ...
    def component_report_with_unit_predecessor(self):
        ...
        for binding in hierarchy["testBindings"]:
            ...
            if binding["testId"] == "tc.p8.formal-test-hierarchy":
                binding["levelId"] = "unit"
        ...
        component = report["executionResult"]["evidenceReferences"][2]
        component.update(evidenceId=component_id, uri=COMPONENT_FIXTURE, sha256=COMPONENT_FIXTURE_SHA256)
        report["evidenceReferences"][2] = component_id
        report["evidenceBindings"][2].update(category="component_fixture", evidenceId=component_id)

        predecessor = copy.deepcopy(self.report_fixture["executionResult"])
        predecessor["testId"] = "tc.p8.formal-test-hierarchy"
        report["predecessorResults"] = [{"assuranceLevel": "unit", "executionResult": predecessor, "evidenceBindings": copy.deepcopy(self.report_fixture["evidenceBindings"])}]
    ...
    def test_deterministic_contract_checks(self):
        ...
        with tempfile.TemporaryDirectory() as directory, self.assertRaises(H.HierarchyError):
            H.write_report(Path(directory), Path("../outside.json"), H.validate_project(PROJECT))

    def test_canonical_registry_runs_enforcement_layer(self):
        mapping = next(item for item in self.registry["affectedTestMappings"] if item["mappingId"] == "affected.p8-formal-test-hierarchy")
        required_paths = {"config/test_center_assurance_report_contract.v1.json", "schemas/test_center_assurance_report_contract.v1.json", "tool/test_center_assurance_enforcement.py", "tool/test_center_assurance_enforcement_test.py"}
        self.assertTrue(required_paths.issubset(set(mapping["pathPatterns"])))
        profiles = {item["stableCheckId"]: item for item in self.registry["projectTestProfiles"]}
        formal = profiles["tc.p8.formal-test-hierarchy"]
        regressions = profiles["tc.p8.formal-test-hierarchy-regressions"]
        self.assertTrue({"config/test_center_assurance_report_contract.v1.json", "schemas/test_center_assurance_report_contract.v1.json", "tool/test_center_assurance_enforcement.py"}.issubset(set(formal["inputPaths"])))
        self.assertEqual(regressions["argv"], ["python", "-m", "unittest", "-v", "tool/test_center_assurance_hierarchy_test.py", "tool/test_center_assurance_enforcement_test.py"])
    ...
    def test_component_accepts_explicit_same_lineage_unit_predecessor(self):
        ...
        self.assertEqual(validated["predecessorEvidenceProofs"][0]["lineageId"], "p8.formal-test-hierarchy")

    def test_component_rejects_unrelated_same_candidate_unit_predecessor(self):
        report, hierarchy = self.component_report_with_unit_predecessor()
        predecessor = report["predecessorResults"][0]["executionResult"]
        predecessor["testId"] = "tc.test-center.semantic-regressions"
        predecessor["moduleId"] = "tm.test-center"
        predecessor["roadmapTaskIds"] = []
        with self.assertRaisesRegex(H.HierarchyError, "unrelated assurance lineage"):
            self.validate_report(report, hierarchy=hierarchy)

    def test_current_evidence_category_swap_is_rejected(self):
        report = copy.deepcopy(self.report_fixture)
        first = report["evidenceBindings"][0]["evidenceId"]
        report["evidenceBindings"][0]["evidenceId"] = report["evidenceBindings"][1]["evidenceId"]
        report["evidenceBindings"][1]["evidenceId"] = first
        with self.assertRaisesRegex(H.HierarchyError, "evidence kind mismatch"):
            self.validate_report(report)

    def test_predecessor_evidence_category_swap_is_rejected(self):
        report, hierarchy = self.component_report_with_unit_predecessor()
        bindings = report["predecessorResults"][0]["evidenceBindings"]
        first = bindings[0]["evidenceId"]
        bindings[0]["evidenceId"] = bindings[1]["evidenceId"]
        bindings[1]["evidenceId"] = first
        with self.assertRaisesRegex(H.HierarchyError, "evidence kind mismatch"):
            self.validate_report(report, hierarchy=hierarchy)
    ...
    def test_predecessor_mistyped_required_evidence_category_is_rejected(self):
        report, hierarchy = self.component_report_with_unit_predecessor()
        report["predecessorResults"][0]["evidenceBindings"][2]["category"] = "component_fixture"
        with self.assertRaisesRegex(H.HierarchyError, "predecessor unit evidence categories"):
            self.validate_report(report, hierarchy=hierarchy)
