...
def _binding(
    ...
    *,
    ...
    if not isinstance(commit, str) or GIT_OBJECT_RE.fullmatch(commit) is None:
        errors.append(f"{label}.{commit_key} must be a 40-character lowercase Git object id")
        return None
    if not isinstance(tree, str) or GIT_OBJECT_RE.fullmatch(tree) is None:
        errors.append(f"{label}.{tree_key} must be a 40-character lowercase Git object id")
        return None
...
def collect_bindings(document: dict[str, Any], errors: list[str]) -> list[tuple[str, str, str]]:
    ...
    if not isinstance(dependencies, list):
        ...
        for index, dependency in enumerate(dependencies):
            ...
            item = _binding(obj.get("implementation"), f"dependency {task_id} implementation", errors)
            if item is not None:
