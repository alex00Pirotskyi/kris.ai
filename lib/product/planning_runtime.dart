...
class RunCoordinator {
  ...
  Future<RunSteeringInstruction> queueSteering(
    ...
    await _bestEffortEvent('steering.queued', runId, <String, dynamic>{
      'runId': runId,
  ...
  Future<_WorkOutcome> _executeWorkItem({
    ...
    var knowledgeContext = deterministicFirstAction != null
    ...
    for (var turn = 0; turn < turnLimit; turn++) {
      ...
      final closedBranches = await repositories.workflow
          .closedAgentActionBranches(
            runId: current.id,
            workItemId: progress.item.id,
            stateSha256: stateSha256,
            limit: 100,
          );
      final closedActionHashes = attemptLedgerPolicy.closedActionHashes(
        closedBranches,
      );
      final closedDecisionHashes = attemptLedgerPolicy.closedDecisionHashes(
        closedBranches,
      );
      ...
      final closedBranchPrompt = attemptLedgerPolicy.closedBranchPrompt(
        closedBranches,
      );
      ...
      final deterministicCandidate = turn == 0
      ...
      if (deterministicSelected) {
        ...
        generation = ModelGenerationResult(
          ...
          inputTokens: 0,
          outputTokens: 0,
          providerDetails: const <String, dynamic>{'deterministicAction': true},
        );
        await _bestEffortEvent(
          ...
          generation = await provider.generate(
            ModelGenerationRequest(
              ...
              onProgress: (modelProgress) {
                ...
                unawaited(
                  _bestEffortEvent('model.${modelProgress.stage}', current.id, <
                    String,
                    dynamic
                  >{
                    'runId': current.id,
      ...
      final decisionSha256 = attemptLedgerPolicy.decisionSha256(
        generation.text,
      );
      ...
      try {
        ...
        final repeatedClosedDecision = closedDecisionHashes.contains(
          decisionSha256,
        );
      ...
      if (action.kind == 'complete') {
        ...
        await repositories.workflow.recordAgentActionAttempt(
          ...
          beforeSha256: stateSha256,
          afterSha256: stateSha256,
          details: <String, dynamic>{'summaryHash': Sha256.text(summary)},
        );
      ...
      final actionJsonForLedger = attemptLedgerPolicy.actionJson(action);
      final actionSha256ForLedger = attemptLedgerPolicy.actionSha256(action);
      final redactedActionForLedger = mapValue(
        redactor.redactJson(actionJsonForLedger),
      );
        ...
        onToolOutput: (tool, stream, delta) {
          liveSignals.publish(
            LiveRunSignal.tool(
              ...
              data: <String, dynamic>{'stream': stream, 'delta': delta},
            ),
          );
        },
      );
      ...
      try {
        ...
        await repositories.workflow.recordAgentActionAttempt(
          ...
          actionSha256: actionSha256ForLedger,
          tool: liveTool,
          outcome: deterministicSelected ? 'deterministic_error' : 'tool_error',
          errorCode: toolError.code,
          beforeSha256: stateSha256,
          afterSha256: stateSha256,
          details: <String, dynamic>{
      ...
      final materialProgress = attemptLedgerPolicy.hasMaterialProgress(
        semanticDelta,
      );
