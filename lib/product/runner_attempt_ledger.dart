...
class RunnerAttemptLedgerPolicy {
  ...
  Future<String> workspaceSha256(
    ...
    int maxFileBytes = 2 * 1024 * 1024,
      ...
      RunnerWorkspaceStateTracker().sha256(
        root,
        maxFiles: maxFiles,
...
class RunnerWorkspaceStateTracker {
  final Map<String, _RunnerWorkspaceFileState> _cache =
      <String, _RunnerWorkspaceFileState>{};

  static const Set<String> _ignoredSegments = <String>{
    '.git',
    '.dart_tool',
    'build',
    'node_modules',
    ...
    Future<void> visit(Directory directory, String prefix) async {
      await for (final entity in directory.list(followLinks: false)) {
        ...
        if (entity is Directory) {
          if (_ignoredSegments.contains(name)) {
            continue;
          }
        ...
        final prior = _cache[relative];
        late final String contentSha256;
        if (prior != null &&
            prior.size == stat.size &&
            prior.modifiedMicros == modifiedMicros) {
          contentSha256 = prior.sha256;
        } else {
          contentSha256 = Sha256.hex(await entity.readAsBytes());
          _cache[relative] = _RunnerWorkspaceFileState(
            size: stat.size,
            modifiedMicros: modifiedMicros,
            sha256: contentSha256,
          );
        }
        entries.add('$relative:$contentSha256');
      }
    }

    await visit(absoluteRoot, '');
    _cache.removeWhere((path, _) => !seen.contains(path));
    entries.sort();
    return Sha256.text(canonicalJson(entries));
  }
}

class _RunnerWorkspaceFileState {
  const _RunnerWorkspaceFileState({
    required this.size,
