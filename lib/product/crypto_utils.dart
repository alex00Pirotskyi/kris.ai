...
class Sha256 {
  ...
  static int _rotr(int value, int bits) => _u32((value >>> bits) | (value << (32 - bits)));
...
class SecretRedactor {
  ...
  void register(String value) {
    ...
    if (trimmed.length >= 4) { _values.add(trimmed); }
  }

  String redact(String input) {
    var output = input;
    final ordered = _values.toList()..sort((a, b) => b.length.compareTo(a.length));
  ...
  Object? redactJson(Object? value) {
    ...
    if (value is Map) {
      return value.map((key, item) {
        ...
        final sensitive = RegExp(r'(secret|token|password|credential|authorization|api.?key)', caseSensitive: false).hasMatch(name);
