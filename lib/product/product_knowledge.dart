...
final class KristinProductKnowledgeRegistry {
  ...
  void register(KristinProductKnowledgeProvider provider) {
    ...
    final descriptors =
        provider.describeProductKnowledge().toList(growable: false);
    // Validate the whole provider before mutating anything, so a rejected
    // provider leaves the registry exactly as it was.
    final declared = <String>{};
    for (final descriptor in descriptors) {
      final id = descriptor.id.trim();
      if (id.isEmpty ||
          descriptor.title.trim().isEmpty ||
          descriptor.summary.trim().isEmpty) {
        throw StateError(
            'invalid_product_knowledge_descriptor:$providerId:$id');
      }
      if (!declared.add(id)) {
        throw StateError(
          'duplicate_product_knowledge_id:$id:$providerId:$providerId',
        );
      }
    ...
    for (final id in declared) {
      _ownerByKnowledgeId[id] = providerId;
    }
  }
