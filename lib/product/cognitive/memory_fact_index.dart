...
final class CognitiveMemoryFactAtom {
  ...
  CognitiveClaim toClaim(MemoryEpisode episode) {
    final diagnostic =
        episode.diagnosticOnly || episode.admission == 'quarantined';
...
''',
    ...
    if (rawFacts is List) {
      for (final raw
          in rawFacts.whereType<Map>().take(maxFactsPerEpisode * 2)) {
        ...
        final predicate =
            _normalizePredicate(item['predicate']?.toString() ?? '');
  ...
  Map<String, List<CognitiveMemoryFactAtom>> cachedForProject(
      String projectId) {
  ...
  void _trim() {
    ...
    final values = _entries.values.toList()
      ..sort((a, b) => b.extractedAt.compareTo(a.extractedAt));
    final keep =
        values.take(maxEntries).map((entry) => entry.episodeId).toSet();
  ...
  factory _MemoryFactCacheEntry.fromJson(Map<String, dynamic> json) =>
      _MemoryFactCacheEntry(
        ...
        facts: (json['facts'] is List
            ...
            .where(
                (item) => item.subject.isNotEmpty && item.predicate.isNotEmpty)
            .toList(growable: false),
      );
}
...
ObservationConfidence _normalizeConfidence(String value) =>
