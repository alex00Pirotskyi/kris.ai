...
''');
  ...
  Future<void> removeRun(String id) => _serialize<void>(() {
    _transaction<void>(() {
      ...
      _database.execute('DELETE FROM run_leases WHERE run_id = ?', <Object?>[
        id,
      ]);
    });
  });
  ...
  Future<List<RunRecord>>
  recoverInFlightRuns() => _serialize<List<RunRecord>>(() {
...
class SqliteEntityRepository<T> implements EntityRepository<T> {
  ...
  Future<void> putAll(Iterable<T> values) => store.putEntities(
    ...
    values.map(
      (value) =>
          MapEntry<String, Map<String, dynamic>>(idOf(value), toJson(value)),
    ),
  );
